...
// === ПАРОЛЬ: измените здесь ===
const APP_PASSWORD = '47474';
// ==============================

// Авторизация теперь на уровне Flask, дополнительные проверки не нужны.
...
const COMPANIES = {
    daneliya: {
        ...
        req: "ИНН: 230410343285<br>ОГРНИП: 317237500425682<br>Расчетный счет: 40802810430000046903<br>Банк: КРАСНОДАРСКОЕ ОТДЕЛЕНИЕ N8619 ПАО СБЕРБАНК<br>БИК: 040349602<br>Корр. счет: 30101810100000000602<br>Адрес: 353460, Краснодарский край, г Геленджик, ул. Волнухина, д.18, кв. 4",
    ...
    alex: {
        ...
        req: "ИНН: 2312328954 | КПП: 231201001<br>ОГРН: 1242300041785<br>Расчетный счет: 40702810947900000107<br>Банк: ФИЛИАЛ \"ЮЖНЫЙ\" ПАО \"БАНК УРАЛСИБ\"<br>БИК: 040349700<br>Корр. счет: 30101810400000000700<br>Юр. адрес: 350065, Краснодарский край, г. Краснодар, ул им. Валерия Гассия, д. 26, кв. 19",
...
function initBlindsTab() {
    // Placeholder for when prices are provided
    console.log("Blinds tab initialized");
}
...
function handleSettingsChange() {
    ...
    updateSillWidths();
    renderCart();
    const comp = COMPANIES[document.getElementById('select-company').value];
    document.getElementById('company-requisites').innerHTML = `<p class="font-bold text-slate-800 text-xs">${comp.name_full}</p><p>${comp.req}</p>`;
    document.getElementById('company-sign-name').innerText = comp.sign_name + " (подпись)";
    document.getElementById('company-contacts').innerHTML = comp.contacts;
}
...
function addFramelessItem() {
    ...
    let panels = parseInt(document.getElementById('fl-panels').value);
    if (!totalW || !h || !panels || panels < 1) { alert('Введите корректные размеры и количество панелей!'); return; }
    ...
    let optionsCost = 0, optionsDesc = [];
    optionsDesc.push(`Цвет профиля: ${ralColor}`);
    optionsDesc.push(`Вид на эскизе: Изнутри`);
    ...
    let flComment = document.getElementById('fl-comment').value.trim();
    if (flComment) optionsDesc.push(`📝 ${flComment}`);
    let flTotal = totalSystemPrice + optionsCost + glassCost;
    ITEMS.push({
        id: Date.now(), category: 'frameless',
        type: `Безрамное остекление (${systemDisplayNames[system]})`,
        qty: 1,
        w: totalW, h: h, area: area, calcArea: area,
        shape: `Панелей: ${panels} шт. по ${panelWidthCm.toFixed(1)} см`,
        isLarge: false, doorPanel: doorPanel, doorDir: doorDir,
        unitCost: flTotal,
        optionsDesc: optionsDesc, baseTotal: flTotal
    });

    document.getElementById('fl-w').value = ''; document.getElementById('fl-h').value = '';
    document.getElementById('fl-panels').value = ''; document.getElementById('fl-door-panel').value = 'none';
...
function renderCart() {
    ...
    let markup = getGlobalMarkup(); let sum = 0;
    if (ITEMS.length === 0) {
        ...
        ITEMS.forEach((it, idx) => {
            let costMult = (it.category === 'custom' || it.category === 'slope_profile') ? 1 : markup;
            let cost = Math.ceil(it.baseTotal * costMult); sum += cost;
            let icon = { glass: 'layout-grid', net: 'grid-3x3', frameless: 'blinds', sill: 'align-horizontal-justify-end', slope: 'square-split-horizontal', slope_profile: 'frame', shower: 'droplets', custom: 'package-plus' }[it.category] || 'box';
            let col = { glass: 'brand-primary', net: 'brand-secondary', frameless: 'slate-800', sill: 'amber-700', slope: 'slate-600', slope_profile: 'slate-500', shower: 'blue-500', custom: 'slate-600' }[it.category] || 'slate-600';
    ...
    // Services sum from SERVICES array
    ...
    let totalManualMarkup = manMarkupRub + (sum * (manMarkupPct / 100));
    let sumWithMarkup = sum + totalManualMarkup;
    let totalDiscount = manDiscRub + (sum * (manDiscPct / 100));
    ...
    document.getElementById('cart-total-display').innerText = Math.ceil(finalTotal).toLocaleString() + ' ₽';
    lucide.createIcons();
}
...
function showProposal() {
    ...
    let markup = getGlobalMarkup();
    let baseSumItems = 0; ITEMS.forEach(it => { baseSumItems += Math.ceil(it.baseTotal * markup); });
    let manMarkupRub = parseFloat(document.getElementById('manual-markup-rub').value) || 0;
    let manMarkupPct = parseFloat(document.getElementById('manual-markup-pct').value) || 0;
    let totalManualMarkupAmount = manMarkupRub + (baseSumItems * (manMarkupPct / 100));
    let markupFactor = baseSumItems > 0 ? 1 + (totalManualMarkupAmount / baseSumItems) : 1;
    let sumItems = 0, totalArea = 0;
    ITEMS.forEach((it, idx) => {
        let costMult = (it.category === 'custom' || it.category === 'slope_profile') ? 1 : markup;
        let baseWithMarkup = Math.ceil(it.baseTotal * costMult);
        let cost = (it.category === 'custom' || it.category === 'slope_profile') ? baseWithMarkup : Math.ceil(baseWithMarkup * markupFactor);
        sumItems += cost; totalArea += it.area * (it.qty || 1);
        ...
        tbody.innerHTML += `<tr class="border-b border-slate-200 hover:bg-slate-50"><td class="p-3 text-slate-500 align-middle">${idx + 1}</td><td class="p-3 align-middle text-center">${sketchHtml}</td><td class="p-3 align-middle"><div class="font-bold text-slate-800 text-sm">${it.type}</div><div class="text-xs text-slate-500 font-medium opacity-80 mt-0.5">${dimLine}</div>${opts}</td><td class="p-3 align-middle text-center font-medium text-slate-700">${qty} ${unitLabel}</td><td class="p-3 align-middle text-right text-xs text-slate-500 whitespace-nowrap">${unitCostDisplay}</td><td class="p-3 align-middle text-right font-bold text-slate-800 whitespace-nowrap">${cost.toLocaleString()} ₽</td></tr>`;
    });
    setTimeout(() => { ITEMS.forEach(it => drawSketch(`sketch-${it.id}`, it)); }, 50);
    let sList = document.getElementById('kp-services-list');
    sList.innerHTML = `<div class="flex justify-between text-sm mb-2 pb-2 border-b border-slate-100"><span class="text-slate-500">Общая площадь изделий:</span><span class="font-bold text-slate-800">${totalArea.toFixed(2)} м²</span></div>`;
    sList.innerHTML += `<div class="flex justify-between text-base mb-1"><span class="text-slate-700 font-bold uppercase tracking-wide">Стоимость изделий:</span><span class="font-black text-slate-900">${sumItems.toLocaleString()} ₽</span></div>`;
    let manDiscRub = parseFloat(document.getElementById('manual-discount-rub').value) || 0;
    let manDiscPct = parseFloat(document.getElementById('manual-discount-pct').value) || 0;
    let totalDiscountAmount = Math.ceil(manDiscRub + (baseSumItems * (manDiscPct / 100)));
    if (totalDiscountAmount > 0) sList.innerHTML += `<div class="flex justify-between text-sm mb-2 text-emerald-700"><span class="font-bold uppercase tracking-wider">Скидка на изделия:</span><span class="font-bold">-${totalDiscountAmount.toLocaleString()} ₽</span></div>`;
    let discountedSumItems = sumItems - totalDiscountAmount;
    if (totalDiscountAmount > 0) sList.innerHTML += `<div class="flex justify-between text-sm mb-3 pb-3 border-b border-slate-300"><span class="text-slate-600 font-medium tracking-wide">Итого за изделия:</span><span class="font-bold text-slate-800"><span class="line-through text-slate-400 mr-2 text-xs">${sumItems.toLocaleString()} ₽</span>${discountedSumItems.toLocaleString()} ₽</span></div>`;
    ...
    let finalKpTotal = discountedSumItems + servSum;
    document.getElementById('kp-total-sum').innerText = finalKpTotal.toLocaleString() + ' ₽';
    let selectedCompany = document.getElementById('select-company').value;
    if (selectedCompany === 'alex') {
        let ndsAmount = finalKpTotal * 22 / 122;
...
function parseWorkbook(workbook) {
    ...
    workbook.SheetNames.forEach(sheetName => {
        ...
        if (sn.includes('стеклопакет')) {
            for (let i = 1; i < rows.length; i++) { if (!rows[i][0]) continue; let val = parseFloat(String(rows[i][1]).replace(/[^0-9.-]+/g, "")); if (!isNaN(val)) { newGlass.push({ name: String(rows[i][0]).trim(), price: val }); hasValidData = true; } }
        } else if (sn.includes('раскладк') || sn.includes('шпрос')) {
            for (let i = 1; i < rows.length; i++) { if (!rows[i][0]) continue; let val = parseFloat(String(rows[i][1]).replace(/[^0-9.-]+/g, "")); if (!isNaN(val)) { newLayouts.push({ name: String(rows[i][0]).trim(), price: val }); hasValidData = true; } }
        } else if (sn.includes('безрам') || sn.includes('salinox')) {
            for (let i = 1; i < rows.length; i++) { if (!rows[i][0] || !rows[i][1]) continue; let sys = String(rows[i][0]).trim(); let maxW = parseFloat(String(rows[i][1]).replace(/[^0-9.-]+/g, "")); let val = parseFloat(String(rows[i][2]).replace(/[^0-9.-]+/g, "")); if (!isNaN(maxW) && !isNaN(val) && newSalinox[sys]) { newSalinox[sys].push({ max: maxW, p: val }); hasValidData = true; } }
        } else if (sn.includes('сетк')) {
            for (let i = 1; i < rows.length; i++) { if (!rows[i][0]) continue; let id = String(rows[i][0]).trim(); let name = String(rows[i][1]).trim(); let p1 = parseFloat(String(rows[i][2] || "0").replace(/[^0-9.-]+/g, "")); let p2 = parseFloat(String(rows[i][3] || "0").replace(/[^0-9.-]+/g, "")); let existing = newNets.find(n => n.id === id); if (existing) { existing.name = name; if (!isNaN(p1)) existing.price_min = p1; if (!isNaN(p2)) existing.price_sqm = p2; hasValidData = true; } }
        } else if (sn.includes('опци') || sn.includes('услуг')) {
            for (let i = 1; i < rows.length; i++) { if (!rows[i][0]) continue; let key = String(rows[i][0]).trim(); let val = parseFloat(String(rows[i][1]).replace(/[^0-9.-]+/g, "")); if (!isNaN(val) && newOptions[key] !== undefined) { newOptions[key] = val; hasValidData = true; } }
        } else if (sn.includes('форм')) {
            for (let i = 1; i < rows.length; i++) { if (!rows[i][0]) continue; let name = String(rows[i][0]).trim(); let val = parseFloat(String(rows[i][1]).replace(/[^0-9.-]+/g, "")); if (!isNaN(val)) { let existing = newShapes.find(s => s.name === name); if (existing) existing.multiplier = val; else newShapes.push({ name: name, multiplier: val }); hasValidData = true; } }
        }
    });
...
// ==========================================
// АРХИВ ПРОСЧЁТОВ (Server API)
// ==========================================
let CURRENT_ORDER_ID = null; // ID текущего открытого заказа из базы
...
function applyState(state, orderId = null) {
    ...
    if (state.kpDate) setVal('kp-date', state.kpDate);
    setVal('kp-manager', state.kpMgr || '');
...
async function saveCalculation() {
    ...
    let currName = [client.trim(), obj.trim()].filter(Boolean).join(' — ') || 'Просчёт';
    
    let name = prompt('Имя просчёта (клиент / объект):', currName);
    if (name === null) return;
    if (!name.trim()) name = currName;
    ...
    const payload = {
        ...
        items: state.items,
        services: state.services,
        markup_rub: state.manualRub || 0,
        markup_pct: state.manualPct || 0,
    ...
    try {
        ...
        if (CURRENT_ORDER_ID) {
            ...
            response = await fetch(`/api/orders/${CURRENT_ORDER_ID}`, {
                method: 'PUT',
        ...
        if (res.success) {
            if (res.id) CURRENT_ORDER_ID = res.id;
            alert(`\u2705 Просчёт "${name.trim()}" сохранён в облаке!`);
            if (document.getElementById('archive-panel').style.display !== 'none') renderArchiveList();
        } else {
            alert('Ошибка при сохранении: ' + (res.message || 'Неизвестная ошибка'));
        }
    } catch (e) {
        console.error(e);
...
async function loadCalculation(id) {
    try {
        ...
        if (!order || order.success === false) return;
        
        if (!confirm(`Загрузить "${order.client_name}"?\n\nТекущая смета будет заменена.`)) return;
        
        const state = {
            items: JSON.parse(order.items_json),
            services: JSON.parse(order.services_json),
            manualRub: order.markup_rub,
            manualPct: order.markup_pct,
            company: order.company,
            kpClient: order.client_name,
            kpDate: order.created_at.split(' ')[0]
        };
        
        applyState(state, order.id);
        closeArchive();
    } catch(e) {
        console.error(e);
...
async function deleteCalculation(id) {
    if (!confirm(`Удалить этот просчёт из архива навсегда?`)) return;
    try {
        const r = await fetch(`/api/orders/${id}`, { method: 'DELETE' });
        const res = await r.json();
        if (res.success) {
            if (CURRENT_ORDER_ID === id) CURRENT_ORDER_ID = null;
            renderArchiveList();
        }
...
async function renameCalculation(id) {
    let name = prompt('Новое имя для просчёта:');
    if (!name || !name.trim()) return;
    
    try {
        // Получаем текущий стейт заказа
        const r = await fetch(`/api/orders/${id}`);
        const order = await r.json();
...
async function renderArchiveList() {
    let container = document.getElementById('archive-list'); if (!container) return;
    container.innerHTML = '<div class="text-center py-8"><i data-lucide="loader-2" class="w-8 h-8 animate-spin mx-auto text-slate-300"></i></div>';
    if (typeof lucide !== 'undefined') lucide.createIcons();
    ...
    container.innerHTML = archive.map(e => {
        ...
        const total = Math.ceil(e.total_amount).toLocaleString('ru-RU');
        return `
        <div class="bg-white border border-slate-200 rounded-xl p-3 hover:border-brand-primary transition-all group ${CURRENT_ORDER_ID === e.id ? 'border-brand-primary bg-brand-light/20' : ''}">
            <div class="flex justify-between items-start gap-2">
                <div class="flex-1 min-w-0">
                    <p class="font-bold text-sm text-slate-800 truncate">${e.client_name}</p>
                    <p class="text-[10px] text-slate-400 mt-0.5">${date} · ${total} ₽</p>
                </div>
            ...
            <button onclick="loadCalculation(${e.id})" class="mt-2 w-full text-xs font-semibold bg-slate-100 text-slate-700 hover:bg-brand-primary hover:text-white py-1.5 rounded-lg transition-all">${CURRENT_ORDER_ID === e.id ? '📍 Сейчас открыт' : '📂 Загрузить'}</button>
        </div>`;
    }).join('');
    if (typeof lucide !== 'undefined') lucide.createIcons();
}
...
function downloadExcelSupplier() {
    const glassItems = ITEMS.filter(item => item.category === 'glass');
    if (glassItems.length === 0) {
        alert('В смете нет стеклопакетов для выгрузки поставщику!');
        return;
    }

    const data = [
        ["Название", "Ширина (мм)", "Высота (мм)", "Кол-во (шт)", "Примечание"]
    ];
    ...
    glassItems.forEach(item => {
        data.push([
            ...
            item.h,
            item.qty,
            item.optionsDesc ? item.optionsDesc.join('; ') : ''
        ]);
    });
    ...
    // Auto-width
    ws['!cols'] = [{ wch: 40 }, { wch: 15 }, { wch: 15 }, { wch: 12 }, { wch: 50 }];
    
    XLSX.utils.book_append_sheet(wb, ws, "Заказ_Поставщику");
    
    const clientName = document.getElementById('kp-client').value || 'Без_имени';
    const fileName = `Заказ_СП_${clientName}.xlsx`;
